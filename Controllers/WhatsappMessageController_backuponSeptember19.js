...
const MessageSend = async (req, res) => {
  try {
    ...
    const tempDesignPath = path.join("uploads", `temp-design-${timestamp}${ext}`);
      ...
      try {
        ...
        if (designFileUrl) {
          try {
            ...
            // 📤 Send button (use both selectors for safety)
            let sendFileBtn = await page.$('button[aria-label="Send"]');
            if (!sendFileBtn) {
              sendFileBtn = await page.$('span[data-icon="send"]');
            }
            if (!sendFileBtn) throw new Error("Send button not found");
            await sendFileBtn.click();
