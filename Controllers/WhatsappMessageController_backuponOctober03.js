...
async function getBrowser() {
  try {
    ...
    browserInstance = await puppeteer.launch({
      ...
      userDataDir: "./whatsapp-session800",
    });
...
const generateHtmlReport = (processed, message) => {
  ...
  <head>
    ...
    <style>
      body { font-family: Arial, sans-serif; background: #ece5dd; margin: 20px; }
  ...
  <body>
    <div class="container">
      ...
      <table>
        <thead>
          <tr><th>Phone Number</th><th>Status</th></tr>
        </thead>
        <tbody>
          ${processed.map(
    (p) => `<tr><td>${p.phone}</td><td class="${p.status}">${p.status}</td></tr>`
  ).join("")}
        </tbody>
      </table>
    </div>
...
const MessageSend = async (req, res) => {
  try {
    ...
    if (!message && !designFileUrl) {
      return res.status(400).json({ status: "error", message: "Message or creative is required" });
    }
    ...
    if (!rawPhones.length) {
      return res.status(400).json({ status: "error", message: "No numbers in CSV." });
    }
    ...
    for (const rawPhone of rawPhones) {
      ...
      try {
        ...
        if (tempDesignPath) {
          try {
            const attachBtn = await page.waitForSelector('span[data-icon="plus-rounded"]', { timeout: 10000 });
            ...
            if (!sendFileBtn) sendFileBtn = await page.$('span[data-icon="send"]');
            if (sendFileBtn) {
              ...
              if (captionBox) {
                ...
                throw new Error("Send button and Enter failed");
              }
            }
          } catch (fileErr) {
            console.error(`❌ Failed creative to ${phone}:`, fileErr.message);
    ...
    // ✅ Generate PDF (separate browser so no conflict)
    try {
